...
function registerCommands(context, outputChannel) {
    ...
    // ⭐️ Visual Editor Command (수정된 부분)
    vscode.commands.registerCommand('labnote.openVisualEditor', (uri) => {
...
async function manageTemplatesCommand(paths) {
    const template = await vscode.window.showQuickPick(logic.getManagableTemplates(paths), { placeHolder: 'Select a template file to manage' });
    if (!template)
        return;
...
function findSectionContext(document, position) {
    ...
    for (let i = sectionLineNum - 1; i >= 0; i--) {
        const lineText = document.lineAt(i).text;
        const uoMatch = lineText.match(/^###\s*\[(U[A-Z]{1,2}\d{3,4})/);
        if (uoMatch) {
